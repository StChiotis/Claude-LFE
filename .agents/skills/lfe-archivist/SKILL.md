...
1. **Read Report**: Read `.plans/inspection_report.md` to understand what was built and verified. **Status branch:**
   ...
   - `status: escalated` (Cycle Guard halt — Brain selected a triage option in the body):
     ...
     - If Brain chose **Option C (Re-plan)**: wipe execution files (`plan_critique.md`, `active_plan.md`, `builder_done.md`, `tdd_report.md`, `.plans/checks/`, `critique.md`, `inspection_report.md`, `diagnosis_report.md`, `rework_directive.md`), keep planning files (`01`/`02`/`03`), drop `.plans/.persona-transition` = `Architect` (the C3 official-transition marker the `persona-transition-guard` checks), then set `Active Persona: Architect`, and loop back to slice re-planning.
...
2. **Sync Docs**: You MUST explicitly apply updates depending on the changes made:
   ...
   - **New architectural/design decisions**: You MUST record them — a **product** decision in `.docs/architecture/architecture-decisions.md`, a **framework** decision in `.docs/architecture/framework-decisions.md`.
...
3.6. **Protocol Debt Resolution** (LFE-FORCE recoveries only): Inspect `.plans/inspection_report.md` frontmatter.
   ...
   - If `source: .docs/quality/PROTOCOL_DEBT.md` AND `status: failed`: leave the entry unresolved and append no findings to `known-issues.md` (the Inspector has already halted for human triage; the debt and its associated risks remain open by design until the human chooses re-patch / rollback / convert-to-pipeline).
