...
1. **Artifact-free**: Reason over the in-context render and write prose only. This skill saves **no image files** — keeping the blank-canvas seal intact and the run self-cleaning. (Render MCPs return the pixels in-context; capture the reasoning, leave the bytes unsaved.)
...
- **2a — preferred renderer**: render the target with an available preview MCP (e.g. a Claude Preview tool). Capture a single in-context render and reason over it.
- **2b — browser renderer**: when the preferred renderer is unavailable, render the route with a browser MCP and reason over the in-context result.
