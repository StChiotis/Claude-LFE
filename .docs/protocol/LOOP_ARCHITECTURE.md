...
3. **Hygiene + reset**: deletes any same-slice `diagnosis_report.md`, resets the `build / tdd / critique / inspect` checkboxes to ⬜, drops `.plans/.persona-transition` = `Builder`, then sets Active Persona to Builder.
...
1. **Arm**: At Sub-Skill Dispatch, the **Visual Floor** auto-arms `lfe-visual-check` because a changed file in `builder_done.md` matches a visual class (`UI_GLOBS`). It renders the surface (preferred renderer → browser → manual instruction), reasons over it, and writes `.plans/checks/visual_findings.md` with a human-action instruction + sign-off token. It is artifact-free (saves no image files).
...
3. **Hard floor**: the `visual-gate` hook (`PreToolUse(Write|Edit)`) denies the Inspector→Archivist transition for a visual slice until both fields are present. It is the framework's first **unconditional-deny** floor (it holds even under `warn` posture), with asymmetric fail-safe ALLOW on every ambiguous path (unreadable substrate, non-visual slice, `escalated`/`failed` status). See framework ADR 102.
