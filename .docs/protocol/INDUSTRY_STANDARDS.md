...
- **visual-gate** (`visual-gate.mjs`, **ADR 102** — a later sibling of this family): denies the Inspector→Archivist close of a *visual slice* until `inspection_report.md` carries `visual_confirmed` + `visual_signoff`. The framework's first **unconditional-deny** floor — it holds even under `warn` posture — kept safe by the same asymmetric fail-safe ALLOW on every ambiguous path (unreadable substrate, non-visual slice, escalated/failed status).

**`Write|Edit` decision order:** boot-precondition → no-mission → persona-transition → visual-gate → persona-path. **Honest ceiling:** this is speed-bumps + loudness, not containment — the harness sandbox is the real boundary; the gates make the cooperative path easiest and drift loud. Promotion to hard-block is per-gate, manual, and human-reviewed against the telemetry log (the visual-gate floor being the one deliberate exception that denies regardless — see ADR 102).
