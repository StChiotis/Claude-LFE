...
graph TD
    ...
    subgraph hygienephase [Phase 5: Hygiene Sub-Pipeline]
        ...
        ArchImprove --> SkillEval[Step 3: lfe-skill-eval]
    end
...
| 8 | `/lfe-inspector` **Finalization Gate** (unified — three outcomes) | `inspection_report.md` (`status: passed`) + `.plans/checks/visual_findings.md` on a visual slice | (a) APPROVE + visual confirmation present → Archivist; (b) APPROVE but visual confirmation absent on a visual slice → obtain `visual_confirmed` + `visual_signoff` then Archivist (backstopped by the `visual-gate` hook); (c) REJECT → `.plans/rework_directive.md` + flip to Builder (rework re-entry, max 5 rounds; GOVERNANCE Limit 3; see `LOOP_ARCHITECTURE.md` Scenario 2.4 + ADR 102) |
...
| 3 | `/lfe-skill-eval` *(every 3rd sweep ≈ 15 sessions, + on-demand)* | `.agents/skills/_evals/**` corpus + the five reasoning skills | `.docs/quality/skill-eval-scorecard.md` + `.claude/lib/__eval__/results.json` |

> **Cadence note (step 3).** The sub-pipeline is *dispatched* every 5 sessions — steps 1–2 run each sweep. `/lfe-skill-eval` is token-heavy (~75 isolated subagent runs), so it **self-gates to every 3rd sweep (≈ 15 sessions)**: it is dispatched each sweep but runs its full pass only when the scorecard's own last-run session is ≥ 15 behind (see [`lfe-skill-eval/SKILL.md`](../../.agents/skills/lfe-skill-eval/SKILL.md) Step 1), and **on-demand** runs (explicit request / pre-release) always execute. "Every 5 sessions" is the sub-pipeline's dispatch trigger, not the eval's own full-run cadence.
