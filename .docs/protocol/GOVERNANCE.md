...
- Architectural decisions are recorded as numbered ADR sections following `ADR-FORMAT.md`: a **product** decision in `.docs/architecture/architecture-decisions.md`, a **framework** decision in `.docs/architecture/framework-decisions.md`. An ADR is only warranted when a decision is hard to reverse, surprising without context, and the result of a real trade-off.
...
| `.docs/architecture/architecture-decisions.md` *(product)* | All `accepted` / `proposed` ADRs + last 15 sessions of `superseded` / `deprecated` | `.docs/archive/architecture-decisions-history.md` | At hygiene sweep (every 5 sessions) |
| `.docs/architecture/framework-decisions.md` *(framework)* | Preserved in full — excluded from archival (all entries Accepted, shipped-machinery rationale) | N/A | N/A |
...
## 🚦 Mechanical Enforcement Gates (Enforcement Hardening — ADR 95)

Beyond the persona path-lock and the Cat-D validators, the framework ships a family of **enforcement gates** that put the rules in the path of the actions that matter. **Doctrine: speed-bumps + loudness, not airtight containment** — the harness sandbox is the real boundary; these gates make off-pipeline drift expensive, visible, and logged, not impossible (aliasing / direct-fs / hook edits bypass by design). All ship **warn-and-log first**, promotable per-gate to hard-block via `.claude/enforcement-posture.json`, each with an **asymmetric fail-safe ALLOW** (an unreadable substrate always allows recovery to proceed).

| Gate | Fires on | Guards |
|---|---|---|
| **C1 — terminal git posture** (`bash-posture-gate.mjs`) | PreToolUse Bash | mutating git needs a mission; merge/push-to-`main`/force/legal-tag also needs typed `MERGE-OK` |
