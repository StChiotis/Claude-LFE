...
> **Note**: Permissions defined in `.agents/permissions.json` are enforced at runtime by the `persona-path-lock` PreToolUse hook (`.claude/hooks/persona-path-lock.mjs`) inside Claude Code. See [`INDUSTRY_STANDARDS.md §6`](INDUSTRY_STANDARDS.md) for the full enforcement-layer picture (PreToolUse hook + optional CI/CD).
...
Available sub-skills (prompt-based reasoning; the five analysis sub-skills are tool-agnostic, while `lfe-visual-check` additionally renders via a preview/browser MCP with a manual fallback):
| Sub-skill | Purpose |
|---|---|
