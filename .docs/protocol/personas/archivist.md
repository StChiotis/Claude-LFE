...
- **Visual-slice backstop**: for a UI-touching slice, the `visual-gate` hook denies the Inspector→Archivist transition until `inspection_report.md` carries `visual_confirmed` + `visual_signoff` — so by the time the Archivist runs, the Brain's visual sign-off is already recorded. The Archivist cleans `visual_findings.md` under `.plans/checks/` like any other sub-skill output (framework ADR 102).
