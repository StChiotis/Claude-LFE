...
| **Bug verification / Finalization** | (1) Reproduction confirmed in `inspection_report.md` (for a diagnosed bug). (2) Regression test exists in the codebase. (3) The finalization gate is resolved: **APPROVE** proceeds to the Archivist; **REJECT** records `.plans/rework_directive.md` (`rework_round` + `directive_hash`) and re-enters the Builder. **For a UI-touching (visual) slice the visual verdict is the Brain's alone**: the Brain looks at the rendered surface (or the named screen) and, on approval, the sign-off is recorded as `visual_confirmed` + `visual_signoff` in `inspection_report.md`; the `visual-gate` hard floor denies the close until both are present (framework ADR 102). |
