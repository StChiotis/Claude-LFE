...
> **Numbering.** These entries keep their original sequence numbers (81 and up) as a frozen historical set; a new framework decision increments the highest number here. The sequence runs 81–103; **ADR 88 (Inspector subagent dispatch) was superseded by ADR 93** before this file was split out, so it is kept as a short stub — its full decision content lives in the index row above and in ADR 93. The Full ADR Bodies section therefore reads 89 → 88 (stub) → 87 by design.

| ADR | Title | Status | What it governs |
|---|---|---|---|
| 103 | Entrance-card contract — retention rule, verify-then-trim, and mechanical guards for `pipeline_status.md` | Accepted | Closes a framework-generic drift class: the Archivist is told to *update* the entrance card but had no rule to *roll* it, so an adopter's card grows into an unbounded mission log on the hottest read path (boot Step 1, statusLine per render, SessionStart, the PreToolUse gate chain — proven downstream at ~96k chars before this contract). **The contract**: the card holds current state only — fixed row labels (the shared `parse-entrance-card.mjs` regex is the consumer contract) · a leading legal `[STATE]` token · pipe-free cells · the `Active Persona` cell as the **bare persona name** (the emoji is statusLine *presentation* — `formatPersona` exact-matches bare, so a decorated cell renders "⚙️ Unknown"; the shipped template carried this latent bug) · ≤ **3** one-line pointers under `## 📜 Recent Missions` · a `## 🧭 Navigation — where what lives` routing table naming a home for every info-class the card deliberately does not hold · ≤ **12,000 chars** hard / > **10,000** warn (CRLF-normalized; framework defaults, project-tunable). **Retention**: a new GOVERNANCE Retention Policy row — cold home is the CHANGELOG lane the Archivist already writes (the card carries pointers only); the Archivist gains Hard Rule 8 + the Step-6 **verify-then-trim** close procedure (append-first refusal: trim only history the CHANGELOG already holds); the Hygiene sweep walks the row as backstop; the Atomic-Docs exemption becomes a closed list. **Guards** (four enforcement moments): write-time warn (`pipeline-status-narrative-check` budget lane) → boot-time banner (`session-start-reminder`, optional-param backward-compatible) → suite-time (the validator's node:test suite incl. an exact-literal budget pin, a budget-boundary CRLF/LF encoding case, and the LIVE template-card guard) → commit-time hard (`scripts/check-entrance-card.mjs` on the shared parser; pre-commit gate 3; `Brain` deliberately absent from its persona set — no write-lane; render-tolerant, enforce-strict vs the statusLine table). Provenance: designed + proven downstream (a private production adopter — a 96k→6k slim with a two-axis parity sweep) and ported generic; numbers map by content across repos (the ADR-95-family convention). |
...
---
## ADR 103: Entrance-card contract — retention rule, verify-then-trim, and mechanical guards for pipeline_status.md (2026-07-14)

**Status:** Accepted

**Context.** `pipeline_status.md` is the framework's hottest read path — `/lfe-boot` Step 1, the statusLine on every render, the SessionStart hook, and the PreToolUse gate chain all parse it — yet it was the one session-mutated file absent from the GOVERNANCE Retention Policy table. The Archivist's close step *prepends* mission summaries; nothing ever rolls them off; the framework's own retention gradient ("`.plans/` is sacred") makes deletion feel like a violation, so cards decay into append-only mission logs. Proven downstream: an adopter's card reached ~96k chars (~43k tokens) of history duplicated verbatim from the CHANGELOG lane before being brought under this contract (a private production adopter's retention ADR). The shipped template card also carried a latent presentation bug: an emoji-decorated `Active Persona` cell, which the statusLine's exact-match `formatPersona` renders as "⚙️ Unknown".

**Decision.** (1) **The Entrance-card contract**: current state only behind the fixed row labels; leading legal `[STATE]` token; pipe-free cells; bare persona name (render-tolerant, enforce-strict — the statusLine table may know extra names like `Brain`, but the validator's persona set is the write-lane roster); ≤ 3 Recent Missions pointer lines; a "where what lives" routing table so a slim card still routes every recovery/lookup; ≤ 12,000 chars hard / > 10,000 warn, counted after CRLF normalization (framework defaults; project-tunable per the Retention Policy's existing tunability clause). (2) **Retention row + verify-then-trim**: the GOVERNANCE table gains the card's row; the Archivist gains Hard Rule 8 and a five-step Step-6 close procedure — verify the CHANGELOG milestone exists (append FIRST if missing; trim only history the CHANGELOG already holds), write the new cells, demote the prior mission to one pointer, drop pointers beyond 3, confirm budget. (3) **Four guard moments**: the narrative-check hook's budget warn lane (write-time, always exit 0), the SessionStart over-budget banner (boot-time, optional third `render()` param — byte-identical output when absent), the node:test validator suite (suite-time; exact-literal budget pin against table-echo circularity, a budget-boundary CRLF/LF equivalence case proving normalization load-bearing, and the LIVE template-card guard), and `scripts/check-entrance-card.mjs` (commit-time hard, pre-commit gate 3, importing the shared parser so validator and consumers can never disagree). (4) The Atomic-Docs exemption becomes a **closed list** (`README.md`, `LLM_AGENT_GUIDE.md`; additions require an ADR); the card is explicitly not exempt.

**Consequences.** Boot/statusLine/gate reads stay O(small) for the life of any adopting project; session hops and crash recovery keep full fidelity (state + routing in the card, work products in `.plans/`, history in the CHANGELOG lane); the template card is contract-conformant out of the box (bare persona, empty Recent-Missions state, routing table). Deletion now has a sanctioned, verifiable moment — the failure mode shifts from silent unbounded growth to a loud commit-time refusal. Residual: prose-procedure deletion in the skill file has no mechanical guard (process-guarded by the Archivist checklist + the Hygiene walk). Provenance: designed and proven on a private downstream production repo; numbers map by content across repos.
