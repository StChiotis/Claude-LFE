...
> **Canonical agent-rules source.** This is the single source of truth for AI-agent behavior in this LFE-adopting repository. The Claude Code adapter at the repo root (`CLAUDE.md`) and the raw Claude.ai chat adapter (`.agents/adapters/system_prompt.txt`) are pointer stubs that reference this file — to change agent behavior, edit *here* (the stubs only point). Projects may declare overrides in dedicated sections (`## Brain Persona Overrides`, `## Retention Policy Overrides`, etc.); the framework respects explicit overrides only.
...
- **Project/Domain Development (Product)**:
  - `.docs/architecture/` (Patterns, coding standards; **product** ADRs in [`architecture-decisions.md`](.docs/architecture/architecture-decisions.md), the **framework's own** ADRs in [`framework-decisions.md`](.docs/architecture/framework-decisions.md))
...
4. **Architecture Decisions** – `.docs/architecture/architecture-decisions.md` (product) · `.docs/architecture/framework-decisions.md` (framework)
...
3. **Execution**: Follow the persona sub-pipelines in strict order:
   - **Architect**: `/lfe-grill-with-docs` → `/lfe-to-prd` → `/lfe-to-issues` → `/lfe-architect` → `/lfe-plan-critique` *(🛡 auto-gate: 5-lens + plan-linter; PASS / WARN / BLOCK; max 2 plan revisions)*
...
4. **Hygiene**: Scheduled every 5 sessions — `/lfe-hygiene` → `/lfe-improve-architecture` run each sweep. `/lfe-skill-eval` is token-heavy, so it self-gates to a longer cadence: **every 3rd sweep (≈ 15 sessions)** + on-demand (see `ASSEMBLY_LINE.md` Phase 5).
...
To preserve the context window, all AI agents are instructed via their system adapters to use extreme brevity (skip pleasantries, keep output minimal).
- **Detailed Answers**: When a detailed explanation or long answer is necessary, write the detailed response to a Markdown file and reply in the chat with a link to the file, rather than emitting it inline.
...
Project-specific overrides go under `## Brain Persona Overrides` in this file. Framework respects explicit overrides only; implicit ones have no effect.

## 8.7 Self-Measurement: the Skill-Accuracy Scorecard

The framework's single self-measurement surface is the **skill-accuracy scorecard** at `.docs/quality/skill-eval-scorecard.md` — a *quality* signal: the measured catch-rate + false-positive rate of the prompt-based reasoning skills, produced by `/lfe-skill-eval`. It measures whether the framework's defect-catching skills actually catch planted defects, directly serving the #1 value, *reliability over tokens*.

Self-measurement is deliberately **quality, not cost**. An earlier per-session token-cost log was retired: it measured the very thing the framework deprioritizes, and its data was an inaccurate agent self-estimate. The scorecard is now the sole surface; run `/lfe-skill-eval` to (re)populate it — see §9 and `ASSEMBLY_LINE.md` Phase 5 for its cadence (every 3rd Hygiene sweep ≈ 15 sessions, or on-demand).

## 8.8 Skill Invocation Authority (CRITICAL)

Skills are **dispatched by the framework**, not by the Brain. Each persona's sub-pipeline (or another skill via chaining) is responsible for invoking the next skill at the right moment, reading the coordination file the previous step left behind. The Brain interacts with you in natural-language intent ("build X", "fix Y", "is this safe?") and at the explicit human-approval gates, leaving implementation skills for the framework to dispatch.
...
## 8.9 Enforcement Layer (ADR 95)

Mechanical enforcement is **speed-bumps + loudness, not containment** (the harness sandbox is the real boundary). Six gates close the structural gaps a momentum-optimizing agent can walk through, all **warn-and-log first** (promotable per-gate via `.claude/enforcement-posture.json`), all with an asymmetric fail-safe ALLOW:
- **C1 terminal git posture** (`bash-posture-gate.mjs`, PreToolUse Bash) — mutating git needs a mission; merge/push-to-`main`/force/legal-tag needs a typed `MERGE-OK`.
...
| `/lfe-plan-critique` | 1.5 | Pre-build 5-lens plan critique (AC, test feasibility, domain, structural, coherence) + mechanical plan-linter |
...
| `/lfe-skill-eval` | 5.3 | Hygiene sub-skill (every 3rd sweep ≈ 15 sessions / on-demand) — skill-accuracy eval runner (isolated subagents, pass@k, scorecard) |
| `/lfe-extract-domain` | Any | Domain knowledge rescue |
| `/lfe-whats-next` | Any | Pipeline navigation |

## 10. Project-Specific Bindings

> This section is populated per-project after the Day 0 domain extraction session.
> Run `/lfe-extract-domain` to produce `CONTEXT.md` and `.docs/domain/domain-knowledge.md`, then document your project's Source of Truth designation, Logic Sovereignty rule, and Domain/Boundary separation here.

### Template placeholders

- **Source of Truth:** see `CONTEXT.md` (repo root) and `.docs/domain/domain-knowledge.md`.
- **Logic Sovereignty:** define the canonical computation boundary for this project.
...
- **`.claude/skills/` mirror for Claude Code dispatch:** Claude Code discovers skills at `.claude/skills/<name>/SKILL.md`. This repo maintains a recursive mirror of `.agents/skills/**` at `.claude/skills/**` so `/lfe-boot` and other Brain-typeable skills dispatch natively. Drift is prevented by a pre-commit hook (`.githooks/pre-commit` calling `npm run sync:lfe-skills:check`); activated per-clone by the `postinstall` script which sets `core.hooksPath = .githooks`. After any LFE upstream merge, run `npm run sync:lfe-skills` then commit. **Edits to skill protocol go to `.agents/skills/` (LFE-SOURCE); the mirror is regenerated by the sync script, not hand-edited.**

### Enforcement bindings

- **Mission-authorized write scope — the `Authorized Scope` entrance-card row** (ADR 95, extends ADR 84). `persona-path-lock.mjs` is mission-aware. When `pipeline_status.md` carries an `| **Authorized Scope** | <glob>, <glob>, … |` row **and** the Mission State is `[IN-FLIGHT: …]`, a Write/Edit whose target matches one of those comma-separated globs is ALLOWED even when it falls outside the active persona's `write_constraints` and outside `FRAMEWORK_INFRA_PATHS` — so a sanctioned mission can write a second repo (e.g. `../OtherRepo/**`) without `LFE-FORCE` or a Protocol-Debt entry. An empty row, the `(none)` placeholder, an absent row, or a non-in-flight mission ⇒ **no extension** (the path-lock behaves exactly as before). The Archivist resets the row to `(none)` at mission close. **Honest ceiling (ADR 95):** the entrance card is agent-editable (Cat II carve-out), so this row is a self-authorization speed-bump, not containment — the harness sandbox is the real boundary.