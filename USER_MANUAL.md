...
3. **Specialist Sub-Skills (opt-in):** If you have enabled them in `.docs/quality/inspector-config.md`, the Inspector also dispatches specialist passes — security (OWASP Top-10), performance, complexity, dependency audit, mutation reasoning, and **visual rendering**. Each writes findings to `.plans/checks/`, then the Inspector aggregates everything into `critique.md`. The visual pass (`lfe-visual-check`) **auto-arms** whenever your change touches a UI file (a stylesheet, template, component, page, or image) — even when you left it off in the config.
...
5. **Finalization (your call):** If it passes the 4-Eyes check, the Inspector states a *technical* pass (logic, baselines, tests) and asks for your blessing — the visual/behavioral verdict is yours.
   - 🛑 **HUMAN ACTION REQUIRED:** Verify the app works visually/functionally, then decide:
     - **Approve** → "Approved, proceed to Archivist." The slice closes.
     - **Reject** (you spot a defect — often a visual one a green test can't catch) → describe what's wrong. The Inspector records it to `.plans/rework_directive.md` and sends the slice **back to the Builder**, which re-implements and re-runs the full Builder → test → inspect cycle. You can reject up to **5 times** per slice (each distinct defect counts); on the 6th the AI stops and offers a safe menu (accept as a known issue / re-plan the slice / fresh mission). The fix always travels back through the Builder — it is never patched in place at the final step.
6. **Visual sign-off (UI changes only) — the hard floor:** When your slice touched a visual file, a green technical pass is not enough to close it. The Inspector renders the changed screen (or, when there is no live preview, names the exact screen for you to open) and asks for your visual verdict:
   - 🛑 **HUMAN ACTION REQUIRED:** Look at the rendered surface. Reply **approve** (it looks right) or **reject** (describe what is wrong).
   - On **approve**, the Inspector records your sign-off — `visual_confirmed` plus a `visual_signoff` token transcribed from your words (the same trust model as your other confirmations) — into `inspection_report.md`, and the slice closes.
   - On **reject**, it routes through the same rework loop as item 5 — back to the Builder, fully re-verified.
   - This is enforced mechanically: the `visual-gate` hook **denies** the close until your sign-off is recorded. It is the framework's one *hard* gate — it holds even in warn mode — yet it steps aside on every ambiguous case (no preview target, an accept-as-debt close), so it cannot trap you.
   - **Tip:** declare your preview surface in the plan with a `## UI Surface` line (a localhost URL or route) so the render is automatic; without it, the check falls back to naming the screen for you to check by hand.
