...
### Step 1.1 — Stamp the Session Boot Marker (C2a)
The boot-precondition gate (ADR 95) treats a session as "booted" only when the boot marker matches the per-session id that the SessionStart hook rotates. Copy the id into the marker so substantive edits this session are recognized as booted:
- Read `.plans/.session-id` (written by the SessionStart hook on session start). If it exists and is non-empty, write its exact content to `.plans/.session-booted`.
- If `.plans/.session-id` is absent (e.g. a harness without the SessionStart hook firing, or a session predating this feature), skip silently — the gate fail-safe-allows when the mechanism is unprimed.

Both files live under `.plans/` (gitignored, transient). This is the only place `/lfe-boot` writes a session marker.
