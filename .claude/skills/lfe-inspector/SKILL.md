...
8. **Handoff**: Once verification passes, ask for human finalization. Upon approval, mark the `inspect ✅` checkbox in `pipeline_status.md`'s Coordination Files row, signal transition to **Archivist**, drop `.plans/.persona-transition` = `Archivist` (the C3 official-transition marker the `persona-transition-guard` checks), then set `Active Persona: Archivist`.
