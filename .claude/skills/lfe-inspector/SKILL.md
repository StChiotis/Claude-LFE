...
6. **Sub-Skill Dispatch** (mission path — for LFE-FORCE recovery branch see Step 7b):
   ...
   d. **Dispatch in fixed order**: `lfe-security-check` → `lfe-perf-check` → `lfe-complexity-check` → `lfe-dep-audit` → `lfe-mutation-verify` → `lfe-visual-check`. **Floors are config-independent minimums**, evaluated when computing the final enabled set: `lfe-security-check` arms on the Security Floor classes, and `lfe-visual-check` arms on the **Visual Floor** — whenever a changed file in `builder_done.md` matches a visual file class (`UI_GLOBS`; see `inspector-config.md`). An override leaves a floored sub-skill armed. For each enabled sub-skill, apply the **resume rule** before invoking:
...
The `visual_confirmed` / `visual_signoff` typed fields ride below `source:` and are tolerated by the base validator with no status-enum change (the ADR-101 `rework_round` precedent). They are absent on a non-visual slice and set at Step 8(b) on a visual one.
...
8. **Finalization Gate (unified — three outcomes)**: Verification passing is a *technical* pass — the Inspector's own authority over logic, baselines, and test coverage. `inspection_report.md` already carries `status: passed` (Step 7), and `checkpoint-flip` has set `inspect ✅`. The *visual* verdict belongs to the Brain alone. First determine whether this is a **visual slice** — any changed file in `builder_done.md` matches a visual file class (`UI_GLOBS`; the Visual Floor will already have armed `lfe-visual-check`, so `.plans/checks/visual_findings.md` is present). Then ask the Brain to finalize and branch:
   - **(a) APPROVE with visual confirmation present** — a non-visual slice, or a visual slice whose `inspection_report.md` already carries `visual_confirmed` + `visual_signoff` → the slice is done. If a `.plans/rework_directive.md` exists for this slice (a prior rework round has now converged), delete it. Drop `.plans/.persona-transition` = `Archivist` (the C3 official-transition marker the `persona-transition-guard` checks), then set `Active Persona: Archivist` and hand off.
   - **(b) APPROVE but visual confirmation absent on a visual slice** → obtain it first. Present the `## Human-Action Instruction` from `.plans/checks/visual_findings.md`; on the Brain's visual approval, **rewrite** `inspection_report.md` via **Write** (so the validator + `checkpoint-flip` fire) recording `visual_confirmed: <ISO-8601>` and `visual_signoff: <token transcribed verbatim from the Brain's approval>`. Then proceed exactly as (a). The `visual-gate` hook backstops this — it denies the Inspector→Archivist transition until both fields are present, so the close stays honest even when this step is skipped.
   - **(c) REJECT** — the Brain finds a defect the technical pass did not catch (most often a visual one) → go to **Step 8b** (the rework re-entry shared with the ADR-101 human-rejection loop).
...
8b. **Rework Re-Entry** (Brain rejected at finalization):
   ...
   7. Reset the per-slice coordination checkboxes (`build / tdd / critique / inspect`) to ⬜ in `pipeline_status.md` — the code is about to change, so the prior technical pass goes stale. (`checkpoint-flip` only advances ⬜→✅, so this reset is explicit, mirroring Partial Cleanup.)
   8. Drop `.plans/.persona-transition` = `Builder` (the C3 official-transition marker), then set `Active Persona: Builder`, and re-enter Phase 2 at the Builder.
   **Guardrail:** the one legitimate path to fix a rejected defect is this persona flip back to the Builder. Editing `src/**` while the Active Persona is Inspector or Archivist is a path-lock violation — it would ship the fix unverified, and the `persona-path-lock` hook denies it.
